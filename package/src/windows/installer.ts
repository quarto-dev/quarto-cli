...
import { emptyDirSync, ensureDirSync, existsSync, moveSync, _createWalkEntry } from "fs/mod.ts";
...
export async function makeInstallerWindows(configuration: Configuration) {
    ...
    if (!sign) {
        configuration.log.warning("No Signing information available in environment, skipping signing");
    }

...
Here is an external [link to Quarto](https://quarto.org) which should be blue (`linkcolor`).

Here is a cross-reference to @fig-test which should be green (`citecolor`).

Here is a cross-reference to @eq-einstein which should also be green.

Here is an internal link to [the introduction](#sec-intro) which should be red (`filecolor`).

See also [the math section](#sec-math) for another internal link example.

## Math Example {#sec-math}
...
![A simple test figure](data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIxMDAiIGhlaWdodD0iMTAwIj48cmVjdCB3aWR0aD0iMTAwIiBoZWlnaHQ9IjEwMCIgZmlsbD0iIzAwZmYwMCIvPjwvc3ZnPg==){#fig-test width=100}

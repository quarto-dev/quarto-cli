...
// PDF text position assertion types
interface PdfTextPositionAssertion {
  subject: string;
  relation: string;
  object: string;
}
...
// Requires pdf.js-extract npm package
export const ensurePdfTextPositions = (
  ...
  return {
    ...
    verify: async (_output: ExecuteOutput[]) => {
      ...
      // 1. Gather all unique search strings from all assertions
      const searchTexts = new Set<string>();
      for (const { subject, object } of assertions) {
        searchTexts.add(subject);
        searchTexts.add(object);
      }
      if (noMatchAssertions) {
        for (const { subject, object } of noMatchAssertions) {
          searchTexts.add(subject);
          searchTexts.add(object);
        }
      }
      ...
      // 3. Check all required texts are found (for positive assertions)
      const missingTexts = assertions
        .flatMap(({ subject, object }) => [subject, object])
        .filter((text) => !foundTexts.has(text));
      if (missingTexts.length > 0) {
        const uniqueMissing = [...new Set(missingTexts)];
        assert(false, `Text not found in PDF ${file}: ${uniqueMissing.map((t) => `"${t}"`).join(", ")}`);
      }

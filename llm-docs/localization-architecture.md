...
- `src/core/language.ts:161-188` — `translationsForLang(language, lang)`: keeps `kLanguageDefaultsKeys`, `crossref-*-title`, `crossref-*-prefix`, and any other non-object (or `null`) value — the last clause is the pass-through for custom/extension keys Quarto does not ship (#14772) — then merges variation sub-objects (`fr-CA` falls back to `fr`).
...
| Pandoc template `$quarto.language.<key>$` (any template, any format) | **2d** | Once the key is in `format.language`, `buildQuartoTemplateVariables` exposes the whole table — no builder-side wiring needed. Step 3 below (adding the key to `kLanguageDefaultsKeys`) is a convention for keys Quarto ships officially, not a runtime gate: any key whose value is a scalar (or `null`), not an object, reaches `format.language` regardless — see the pass-through checks at `src/core/language.ts:174-180` (`translationsForLang`) and `:119-123` (`readLanguageTranslations`). Templates then read `$quarto.language.<key>$`. |
...
- **Custom/extension keys under `language:` pass through flat and unnamespaced.** There is no `quarto.extensions.<name>.language.<key>` scheme (#14772/#14773 decided this in favor of the simpler flat form). A scalar or `null`-valued key that is not in `kLanguageDefaultsKeys` still survives `translationsForLang` and, for a locale-variation file, `readLanguageTranslations` — both treat "not an object" as "user or extension defined string" rather than dropping it. Only object-valued keys are treated as locale sub-blocks and excluded from this pass-through.
- **To clear a shipped key's default, set it to `""`, not `null`.** All shipped `format-language` keys in `src/resources/schema/definitions.yml` are plain `string` — `null` fails schema validation for these keys (a YAML key written with nothing after the colon parses as `null`, which is usually a typo, not an intentional clear). This is deliberate: nothing in `_language.yml` or the test suite relies on `null` clearing a shipped default, and the pass-through logic above only exists for custom/extension keys Quarto does not ship. #14813 made the same call for its five `navigation-*-label` keys; #14773 briefly widened 60 shipped keys to `anyOf: [string, null]` to stop the typo case from erroring, but that traded a caught mistake for a worse one — the label silently rendering as an empty string with no fallback — so those 60 keys were reverted back to plain `string`.

...
- ([#14577](https://github.com/quarto-dev/quarto-cli/issues/14577)): Fix an embedded notebook with no explicit title using a heading that isn't its first content (such as a code-cell comment) as the notebook title. The embedded title now follows the same rule as the notebook's own title — a heading is only used when nothing precedes it — and otherwise falls back to the filename.

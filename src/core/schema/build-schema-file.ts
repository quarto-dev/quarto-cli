...
export async function buildIntelligenceResources() {
  ...
  await generateTypesFromSchemas();
}

async function generateTypesFromSchemas() {
  const definitionsSchema = parse(
    Deno.readTextFileSync(resourcePath("/schema/definitions.yml")),
    // deno-lint-ignore no-explicit-any
  ) as any;
  const projectSchema = parse(
    Deno.readTextFileSync(resourcePath("/schema/project.yml")),
    // deno-lint-ignore no-explicit-any
  ) as any;
  ...
  Deno.mkdirSync(resourcePath("/types"), { recursive: true });
  Deno.writeTextFileSync(
    resourcePath("/types/schema-types.ts"),
    autoGeneratedTypes.join("\n\n"),
  );

  await Deno.run({
    cmd: [Deno.execPath(), "fmt", resourcePath("/types/schema-types.ts")],
  }).status();
}

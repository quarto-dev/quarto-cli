...
// break-quarto-md.ts
async function breakQuartoMd(src, validate = false) {
  const nb = {
    cells: []
  };
  const yamlRegEx = /^---\s*$/;
  ...
  let language = "";
  const lineBuffer = [];
  const flushLineBuffer = async (cell_type) => {
    if (lineBuffer.length) {
      ...
      if (cell_type === "code" && (language === "ojs" || language === "dot")) {
        const { yaml, source: source2, sourceStartLine } = await partitionCellOptionsMapped(language, cell.source, validate);

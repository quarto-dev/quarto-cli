...
function scoped_walk(outer_node, filter)
  ...
  local function process_handler(handler, node)
    ...
    if handler then
      ...
      if result ~= nil then
        ...
        result = inner(result)
        context:insert(nil) -- a dummy value to keep the context management consistent
        return result, true
      end
    end
    return nil, false
  ...
  local function process_blocks(blocks)
    local result, done = process_handler(filter[nt], blocks)
    if done then
  ...
  local function process_inlines(inlines)
    local result, done = process_handler(filter[nt], inlines)
    if done then
      return result or inlines
    end
    assert(result == nil)

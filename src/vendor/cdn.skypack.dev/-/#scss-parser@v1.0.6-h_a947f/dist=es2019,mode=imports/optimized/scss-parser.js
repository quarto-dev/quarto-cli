...
class InputStream {
  ...
  peek(offset) {
    const cursor = lodash.isInteger(offset) ? this.cursor + offset : this.cursor;
...
const is_comment_start = (input) => input.peek() === "/" && (input.peek(1) === "/" || input.peek(1) === "*");
const is_ident_start = (c) => /[a-z_]/i.test(c);
const is_ident = (c) => /[a-z0-9_-]/i.test(c);
const is_number_start = (input) => is_digit(input.peek()) || input.peek() === "." && is_digit(input.peek(1));
const is_hex = (input) => {
  let hex = input.peek();
  if (hex === "#") {
    let _3 = false;
    ...
    while (hex.length < 7) {
      ...
      hex += c;
      if (hex.length === 4)
        _3 = HEX_PATTERN.test(hex);
      if (hex.length === 7)
        _6 = HEX_PATTERN.test(hex);
    }
...
class TokenStream {
  constructor(input) {
    invariant2(lodash.isPlainObject(input) && lodash.has(input, "next"), "TokenStream requires an InputStream");
    this.input = input;
    this.tokens = [];
  }
  createToken(type2, value, start) {
    return Object.freeze({
      type: type2,
      value,
      start,
  ...
  read_string(c) {
    ...
    if (c === "'")
      type2 = "string_single";
    return this.createToken(type2, value, start);
  }
  read_number() {
    const start = this.input.position();
    let hasPoint = false;
...
class Parser {
  ...
  createNode(type2, value, start, next) {
    return {type: type2, value, start, next};
  }
  is_type(type2, ...values) {
    const t = this.tokens.peek();
    if (!values.length)
      return t ? type2.test(t.type) : false;
    return values.reduce((a, c, i) => {
      const t2 = this.tokens.peek(i);
      return !t2 ? false : a && type2.test(t2.type) && t2.value === c;
    }, true);
  }
  is_space() {
  ...
  parse_node() {
    if (this.is_space() || this.is_comment())
      return this.tokens.next();
    const value = [];
    const maybe_declaration = (punctuation) => {
      let expandedPseudo = false;
      if (punctuation === ";") {
        const pseudoIndex = lodash.findIndex(value, {
          type: "pseudo_class"
        });
        if (pseudoIndex > 0) {
          const a = value[pseudoIndex];
          const b = this.createNode("punctuation", ":", a.start, lodash.first(a.value).start);
          const nodes = [b].concat(a.value);
          value.splice(pseudoIndex, 1, ...nodes);
          expandedPseudo = true;
        }
      }
      const puncIndex = lodash.findIndex(value, {
        type: "punctuation",
        value: ":"
      });
      if (puncIndex >= 0) {
        ...
        if (maybeSpace.type === "space" || expandedPseudo) {
          const start = lodash.first(value).start;
          let next = lodash.last(value).next;
          const property_ = lodash.take(value, puncIndex);
          const propertyNode = this.createNode("property", property_, lodash.first(property_).start, lodash.last(property_).next);
          const value_ = lodash.drop(value, puncIndex + 1);
          if (punctuation === "{") {
            const block = this.parse_block();
            value_.push(block);
            next = block.next;
          }
          const valueNode = this.createNode("value", value_, lodash.first(value_).start, lodash.last(value_).next);
          ...
          return this.createNode("declaration", declarationValue, start, next);
        }
      }
      return false;
    };
  ...
  parse_declaration(property) {
    ...
    const propertyNode = this.createNode("property", property, lodash.first(property).start, lodash.last(property).next);
    const valueNode = this.createNode("value", value, lodash.first(value).start, lodash.last(value).next);
    const declarationValue = [propertyNode, firstSeparator, valueNode];
    if (secondSeparator)
      declarationValue.push(secondSeparator);
    return this.createNode("declaration", declarationValue, lodash.first(property).start, lodash.last(value).next);
  }
  parse_wrapped(type2, open, close, _start) {
    const {start} = this.skip_punctuation(open);
    const value = this.parse_expression(() => !this.is_punctuation(close));
    const {next} = this.skip_punctuation(close);
    return this.createNode(type2, value, (_start || start).start, next.next);
  }
  parse_block() {
  ...
  parse_arguments(type2) {
    const {start} = this.skip_punctuation("(");
    let value = [];
    if (type2 === "pseudo_class") {
      while (!this.tokens.eof() && !this.is_punctuation(")")) {
        value.push(this.parse_atom());
      }
    } else {
  ...
  maybe_function(node) {
    ...
    return this.is_punctuation("(") && lodash.includes(types, node.type) ? this.parse_function(node) : node;
  }
  parse_function(node) {
  ...
  parse_at_rule() {
    ...
    return this.createNode("atrule", value, start.start, lodash.last(value).next);
  }
  parse_rule(selectors) {
    const selector = this.createNode("selector", selectors, lodash.first(selectors).start, lodash.last(selectors).next);
    const block = this.parse_block();
    return this.createNode("rule", [selector, block], selector.start, block.next);
  }
  parse_selector(type2, punctuation) {
    ...
    if (!value.length) {
      this.tokens.err(`Selector ("${type2}") expected "identifier" or "interpolation"`);
    }
    return this.createNode(type2, value, start.start, lodash.last(value).next);
  }
}
var parse = (tokenStream2) => {
  invariant2(lodash.isPlainObject(tokenStream2) && lodash.has(tokenStream2, "next"), "Parser requires a TokenStream");
  const parser = new Parser(tokenStream2);
...
const walkNode = (node) => {
  ...
  if (lodash.isString(node.value))
    return node.value;
  if (lodash.isArray(node.value))
    return walkValue(node.value);
  return "";
};
...
var stringify = (node) => walkNode(node);
const parse$1 = (css) => parse(tokenStream(inputStream(css)));
const stringify$1 = (node) => stringify(node);
var lib = {
  parse: parse$1,
  stringify: stringify$1
};
export default lib;
var parse$2 = lib.parse;
var stringify$2 = lib.stringify;
export {lib as __moduleExports, parse$2 as parse, stringify$2 as stringify};
